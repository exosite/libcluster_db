defmodule ClusterDB.Strategy.Mongo do
  ...
  def init([%State{} = state]) do
    ...
    mongodb_type =
      case get_setting(mongodb, :type) do
    ...
    {:ok, _} = Mongo.start_link(mongo_opts)
                  ...
                  %{
                    ...
                    | meta: %{
                        ...
                        | last_timestamp: timestamp,
                          nodes_scan_job:
                            spawn_monitor(fn ->
                              :erlang.exit(scan_nodes(timestamp, max_heartbeat_age, meta))
                            end)
                      }
                  }
                }
  ...
  defp scan_nodes(
    ...
    {good_nodes_map, bad_nodes_map} =
      ...
      |> Enum.reduce(
        ...
        fn
          %{"node_id" => node_id, "timestamp" => node_timestamp}, {good_acc, bad_acc}
